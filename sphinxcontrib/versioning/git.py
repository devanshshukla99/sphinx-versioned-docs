...
RE_REMOTE = re.compile(
    r"^(?P<sha>[0-9a-f]{5,40})\trefs/(?P<kind>heads|tags)/(?P<name>[\w./-]+(?:\^\{})?)$",
    re.MULTILINE,
)
...
def run_command(local_root, command, env_var=True, pipeto=None, retry=0, environ=None):
    ...
    with open(os.devnull) as null:
        main = Popen(
            ...
            env=env,
            stdout=PIPE,
            stderr=PIPE if pipeto else STDOUT,
            stdin=null,
        )
        if pipeto:
            pipeto(main.stdout)
            main_output = main.communicate()[1].decode(
                "utf-8"
...
def get_root(directory):
    ...
    try:
        ...
        raise GitError(
            "Failed to find local git repository root in {}.".format(repr(directory)),
            exc.output,
        )
    if IS_WINDOWS:
    ...
    if "^{}" in output:
        ...
        for group in (m.groupdict() for m in RE_REMOTE.finditer(output)):
            ...
            if (
                dereferenced
                and parsed
                and kind == parsed[-1]["kind"] == "tags"
                and name == parsed[-1]["name"]
            ):
...
def export(local_root, commit, target):
            ...
            with tarfile.open(fileobj=stdout, mode="r|") as tar:
                for info in tar:
                    ...
                    if not path.startswith(target):  # Handle bad paths.
                        log.warning(
                            "Ignoring tar object path %s outside of target directory.",
                            info.name,
                        )
    ...
    for file_path in mtimes:
        last_committed = int(
            run_command(
                local_root,
                ["git", "log", "-n1", "--format=%at", commit, "--", file_path],
            )
        )
...
def clone(local_root, new_root, remote, branch, rel_dest, exclude):
    ...
    try:
        run_command(
            new_root,
            ["git", "clone", remotes[remote][0], "--depth=1", "--branch", branch, "."],
        )
    except CalledProcessError as exc:
        ...
        try:
            run_command(
                ...
                [
                    "git",
                    "remote",
                    "set-url" if name == "origin" else "add",
                    name,
                    fetch,
                ],
                retry=3,
