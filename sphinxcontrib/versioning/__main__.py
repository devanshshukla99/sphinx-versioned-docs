...
def build_options(func):
    ...
    func = click.option(
        "-p",
        "--priority",
        type=click.Choice(("branches", "tags")),
        help="Group these kinds of versions at the top (for themes that don't separate them).",
    )(func)
    func = click.option(
        "-r",
        "--root-ref",
    ...
    func = click.option(
        ...
        "--sort",
        multiple=True,
        type=click.Choice(("semver", "alpha", "time")),
        help="Sort versions. Specify multiple times to sort equal values of one kind.",
    )(func)
    func = click.option(
        "-t",
        "--greatest-tag",
...
def override_root_main_ref(config, remotes, banner):
    ...
    if greatest_tag or recent_tag:
        ...
        if candidates:
            ...
            config.update(
                {"banner_main_ref" if banner else "root_ref": candidates[0]["name"]},
                overwrite=True,
            )
        else:
